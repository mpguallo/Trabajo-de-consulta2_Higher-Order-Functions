...
def integracion(funcion: Double => Double, a: Double, b: Double): Double = {
  val puntoMedio: Double = (a + b) / 2
  val f1: Double = funcion(a) // Evaluación en el límite inferior
  val f2: Double = funcion(b) // Evaluación en el límite superior
  val fMedio: Double = funcion(puntoMedio) // Evaluación en el punto medio
...
val resultado1: Double = integracion(x => -Math.pow(x, 2) + 8 * x - 12, 3, 5)
...
val resultado2: Double = integracion(x => 3 * Math.pow(x, 2), 0, 2)
...
val resultado3: Double = integracion(x => x + 2 * Math.pow(x, 2) - Math.pow(x, 3) + 5 * Math.pow(x, 4), -1, 1)
...
val resultado4: Double = integracion(x => (2 * x + 1) / (Math.pow(x, 2) + x), 1, 2)
...
val resultado5: Double = integracion(x => Math.exp(x), 0, 1)
...
val resultado6: Double = integracion(x => 1 / Math.sqrt(x - 1), 2, 3)
...
val resultado7: Double = integracion(x => 1 / (1 + Math.pow(x, 2)), 0, 1)
